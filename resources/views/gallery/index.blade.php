    ...
    <div class="relative min-h-screen"
        ...
        <header class="flex items-center justify-between px-6 py-4">
            ...
            <div class="flex items-center gap-4">
                ...
                @else
                    <a href="{{ route('login') }}" class="text-small text-content-3 hover:text-content">Log in</a>
                @endauth
            </div>
        </header>
        ...
        <main class="mx-auto max-w-3xl px-6 py-10">
            <div class="mb-6">
                <p class="font-mono text-label uppercase tracking-[0.06em] text-content-4 mb-1">Fastest to the stars</p>
                <h1 class="font-display font-bold text-h1 text-content">Gallery</h1>
                <p class="text-small text-content-3 mt-1">Every lineage here finished its whole journey, and its player chose to share it. Nothing here is tied to an account you can browse — just the finished lineages themselves.</p>
            </div>
                        ...
                        @foreach ($leaderboard as $entry)
                            <div class="flex items-center gap-4 py-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-small text-content-4">
                                        {{ $entry['ending']['name'] ?? 'Unknown ending' }} · {{ $entry['adaptations'] }} adaptations
                                        @if ($entry['isChallenge']) · <span class="text-content-3">Challenge run</span> @endif
                                    </p>
                                </div>
            ...
            @else
                <x-ui.panel class="p-7 text-center">
                    <h2 class="font-display font-bold text-h3 text-content mb-1">No lineages listed yet</h2>
                    <p class="text-small text-content-2">Finish a lineage and choose to join the gallery, and it'll appear here.</p>
                </x-ui.panel>
            @endif
        </main>
    </div>
